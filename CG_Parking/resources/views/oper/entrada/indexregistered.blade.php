...
<ol class="breadcrumb">
    ...
    <li><a href="{{ route('entradas.index') }}"><strong>Entrada de Veículo</strong></a></li>
    <li><a href="{{ route('entradas.index') }}"><strong>Cliente Cadastrado</strong></a></li>
</ol>
@stop
...
@endif
@if (session('mensagem_error_cliente'))

<div class="alert alert-danger alert-dismissible">
        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
        <h4><i class="icon fa fa-ban"></i>{{ session('mensagem_error_cliente') }}</h4>
...
<!-- /.box-header -->
    <div class="box-body">
        <form role="form" method="POST" action="{{ route('entradas.show') }}">
            {!! csrf_field ()!!}
            <div class="input-group input-group-sm pull-right" style="width: 200px;">
                    <input type="text" name="table_search" class="form-control pull-right" placeholder="Buscar por nome de cliente">
                ...
                <div class="box-body">
                    <form role="form" method="POST" action="{{ route('entradas.store') }}">
                        {!! csrf_field ()!!}
                        <div class="form-group">
                            <label for="cliente">Cliente</label>
                            <select size="4" name="cliente" class="form-control">
                                <option value="0" selected>Selecione:</option>
                                @foreach($user as $e)
                                    <option value="{{$e -> id}}">Nome do Cliente: {{$e -> name}} | Endereço: {{$e -> endereco}} | CPF: {{$e -> cpf}} | E-mail: {{$e -> email}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
